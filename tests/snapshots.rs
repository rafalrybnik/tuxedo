...
const FIXTURE_PATH: &str = "/tmp/tuxemdo-snapshot.txt";
...
const FIXTURE_CONFIG_PATH: &str = "/tmp/tuxemdo-snapshot.toml";
...
fn insert_dialog_after_nl_parse() {
    ...
    assert_eq!(outcome, tuxemdo::app::AddOutcome::Parsed);
}
...
fn list_scrolls_to_keep_cursor_visible_when_below_fold() {
    ...
    while app.prefs.sort != tuxemdo::app::Sort::File {
        app.cycle_sort();
    }
