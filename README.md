...
- **TUI and CLI in one binary.** Run `tuxemdo` for the interactive UI, or `tuxemdo <command>` for a [todo.txt-cli](https://github.com/todotxt/todo.txt-cli)-compatible command line (`add`, `ls`, `do`, `pri`, `archive`, …) — scriptable, with `--json` output and `$TODO_DIR` / `$TODO_FILE` / `$DONE_FILE` support.
- **Natural-language add.** Type prose into the add prompt — `Pay rent monthly on the first, show 3 days before due, project home` — and tuxemdo rewrites it to canonical todo.txt for you to review and save. Local, offline, no AI service.
...
- **Atomic, sync-friendly writes.** Every change goes through write-temp-then-rename. If another process — Dropbox, an editor, a script — modifies the file, tuxemdo reloads on the next keypress (or within ~250 ms while idle) and flashes a notice.
...
Beyond the built-ins, tuxemdo loads any `*.toml` file you drop in
`${XDG_CONFIG_HOME:-$HOME/.config}/tuxemdo/themes/`. Each one joins the `T`
picker in sorted filename order. Ready-made themes live in
[`docs/themes/`](docs/themes) — copy one in and press `T`:

```sh
mkdir -p ~/.config/tuxemdo/themes
curl -o ~/.config/tuxemdo/themes/gruvbox-dark-soft.toml \
...
Download the archive for your platform from the [latest release](https://github.com/webstonehq/tuxedo/releases/latest) and put `tuxemdo` on your `PATH`.
...
`tuxemdo` is two things in one binary: an interactive TUI, and a one-shot
...
```sh
tuxemdo [FILE]      # launch the TUI on FILE (created if missing)
tuxemdo             # TUI on the default file (see resolution below)
tuxemdo --sample    # open the bundled sample file in the temp dir
tuxemdo <command>   # run a one-shot CLI command — see "Command-line interface"
tuxemdo update      # print upgrade instructions for your install
tuxemdo --help
tuxemdo --version
```

When a newer release is available, the status bar shows `↑ <version> (tuxemdo
update)` next to the version. The check runs in the background, is cached at
`$XDG_CACHE_HOME/tuxemdo/latest_version.json` for 24 h, and fails silently
when offline. Set `TUXEMDO_NO_UPDATE_CHECK=1` to disable.

### Which file tuxemdo opens
...
tuxemdo notices on the next keypress, or within ~250 ms while idle, and
...
When the first argument is a recognized subcommand, tuxemdo runs a one-shot
...
```sh
tuxemdo add "Pay rent +home @bank due:2026-07-01"   # or: tuxemdo a "..."
tuxemdo ls @bank                                     # filter by context
tuxemdo do 3                                          # mark task 3 complete
tuxemdo pri 3 A                                        # set priority
tuxemdo archive                                        # move done tasks to done.txt
tuxemdo ls --json | jq .                              # machine-readable output
```
...
Global flags may appear before the subcommand (`tuxemdo -f del 3`).
...
press `A` in the TUI), matching tuxemdo's interactive model. There is no `-d`
config-file flag; configure paths with the environment variables above.

## Keybindings

Custom normal-mode keybindings can be added in
`${XDG_CONFIG_HOME:-$HOME/.config}/tuxemdo/keybinds.toml`:
...
- `rec:[+]N{d,b,w,m,y}` — recurrence; on completion (`x`), tuxemdo inserts
...
the buffer already contains a `due:`, `rec:`, or `t:` token, tuxemdo assumes
...
`inbox.txt`, which tuxemdo drains on every external-change poll: each line
...
to the sibling `inbox.txt`, tuxemdo picks it up.
...
Drains from tuxemdo-managed producers are crash-safe: the capture server
...
Persisted to `${XDG_CONFIG_HOME:-$HOME/.config}/tuxemdo/config.toml`. Cycling
...
- [ratatui](https://ratatui.rs/) and [crossterm](https://github.com/crossterm-rs/crossterm) — the rendering and terminal-input crates tuxemdo is built on.
