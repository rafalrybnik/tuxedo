...
//!
//! Path: `${XDG_CONFIG_HOME:-$HOME/.config}/tuxemdo/config.toml`
//!
...
impl Config {
    ...
    /// Resolve `${XDG_CONFIG_HOME:-$HOME/.config}/tuxemdo/config.toml`.
    ...
    pub fn path_in(xdg_base: &Path) -> PathBuf {
        xdg_base.join("tuxemdo").join("config.toml")
    }
}
...
fn serialize(c: &Config) -> String {
    let mut out = String::from("# tuxemdo config\n");
...
mod tests {
    ...
    fn save_then_load_via_explicit_path() {
        ...
        assert!(path.ends_with("tuxemdo/config.toml"));
