//! Network helpers for `tuxemdo serve`: LAN-IP discovery, token
...
mod tests {
    ...
    fn append_rejects_empty_line() {
        let dir =
            std::env::temp_dir().join(format!("tuxemdo-serve-append-empty-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let todo_path = dir.join("todo.md");
        std::fs::write(&todo_path, "").unwrap();
    ...
    fn append_inserts_leading_newline_when_prior_content_lacks_one() {
        ...
        let dir =
            std::env::temp_dir().join(format!("tuxemdo-serve-append-noeol-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let todo_path = dir.join("todo.md");
        std::fs::write(&todo_path, "").unwrap();
