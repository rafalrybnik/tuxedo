...
//!
//! Path: `${XDG_CONFIG_HOME:-$HOME/.config}/tuxemdo/keybinds.toml`
//!
...
impl KeyBindings {
    ...
    pub fn path_in(xdg_base: &Path) -> PathBuf {
        xdg_base.join("tuxemdo").join("keybinds.toml")
    }
...
mod tests {
    ...
    #[test]
    fn path_uses_tuxemdo_keybinds_toml() {
        let path = KeyBindings::path_in(Path::new("/tmp/config"));
        assert!(path.ends_with("tuxemdo/keybinds.toml"));
    }
}
