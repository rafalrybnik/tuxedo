...
use tuxemdo::action::Action;
use tuxemdo::app::{AddOutcome, App, CalendarTarget, DialogInputMode, Mode, OverlayKind, View};
use tuxemdo::cli;
use tuxemdo::config::Config;
use tuxemdo::keybinds::{KeyBindings, ResolvedKey};
use tuxemdo::theme;
use tuxemdo::ui::hyperlinks;
use tuxemdo::{clipboard, todo, ui, update};
...
fn main() -> Result<()> {
    ...
    if let Some(code) = tuxemdo::cmd::run(&argv)? {
    ...
    match arg.as_deref() {
        ...
        Some("--version") | Some("-V") => {
            println!("tuxemdo {}", env!("CARGO_PKG_VERSION"));
            return Ok(());
        }
        Some("update") => {
            update::run()?;
    ...
    match theme_warnings.len() {
        ...
        n => app_state.flash(format!(
            "{n} theme(s) skipped — check ~/.config/tuxemdo/themes/"
        )),
    }
    if std::env::var_os("TUXEMDO_NO_UPDATE_CHECK").is_none() {
    ...
    if app_state.mode != Mode::Welcome {
        eprintln!("tuxemdo: {}", app_state.file_path.display());
    }
    result
}
...
fn print_usage() {
    ...
    println!("  update                    print instructions for upgrading tuxemdo");
    println!();
    println!("Options:");
...
// `Action` lives in `tuxemdo::action` (see `src/action.rs`). Keeping it in the
...
mod tests {
    ...
    fn welcome_s_opens_sample_and_enters_normal() {
        ...
        assert!(
            app.file_path.ends_with("tuxemdo-sample.txt"),
    ...
    fn build_app_with_archive(todo_raw: &str, done_raw: Option<&str>) -> App {
        ...
        let dir =
            std::env::temp_dir().join(format!("tuxemdo-bindings-{}-{}", std::process::id(), n));
